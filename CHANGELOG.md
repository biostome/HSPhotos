...
- **全量折叠/展开**：底栏快捷按钮不再仅作用于可见屏幕，改为作用于**全量 assets**——折叠取最深未折叠层级，展开取最浅已折叠层级，一次一整级。
- **O(n) 性能**：全量算法直接基于 `levels` 字典按层级分组，避免原可见区域限定算法在全量场景下的 O(n²) 退化，大量照片也流畅不卡。

### 🎯 选择模式下折叠/展开选中项

- **选中层级项后自动出现按钮**：选择模式下选中含层级照片，底栏工具栏自动出现折叠/展开按钮；操作仅作用于选中项。取消选中后按钮自动消失。
- 选择模式底栏同时保留**上一处/下一处**快速跳转和**隐藏无级照片**按钮。

### 👁️ 一键隐藏无级照片

- 底栏新增 **eye.slash / eye** 按钮，切换隐藏/显示 `level == 0` 的无层级照片。普通模式和选择模式均可用。
- 动画与折叠/展开共用同一套 batch update / cross-dissolve 路径，`hierarchyNumbersUnchanged: true` 跳过不必要的全量编号重算，避免卡顿。
- 隐藏时仅显示已编号照片，按钮变为 `eye` 图标、标签为「显示全部」。

### 🔗 链式快捷操作

- **折叠到底 → 自动隐藏无级照片**：连续点击折叠直到全部收起后，再次点击折叠自动将无级照片隐藏。
- **展开优先恢复无级照片**：无级照片隐藏时，点击展开优先恢复显示全部照片，再次点击展开才开始逐级展开层级。

### 📝 文档

- `README.md` 5.7 节、12.9b 节新增层级快捷按钮、隐藏无级照片、链式操作的完整产品说明。
- `CHANGELOG.md` 同步更新。
